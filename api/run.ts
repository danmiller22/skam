...
/**
 * Lalafo → Telegram бот под Vercel.
 *
 * Условия:
 *  - город: только Бишкек
 *  - 1–2 комнаты
...
// аккуратный лимит объявлений и страниц
...
// простая in-memory защита от дублей на тёплом рантайме
  ...
  for (let attempt = 1; attempt <= 2; attempt++) {
    ...
    if (res.status === 429) {
      ...
      try {
        ...
        // @ts-ignore
        const retry = data.parameters?.retry_after ?? data.retry_after;
