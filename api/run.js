...
// In-memory "seen" cache for this lambda instance
const seen = new Set();
...
function extractListingLinks(html, citySlug) {
...
function parseOwnerName(html) {
  let m = html.match(/"sellerName"\s*:\s*"([^"]+)"/);
  if (m && m[1]) return m[1];

  m = html.match(/"userName"\s*:\s*"([^"]+)"/);
  if (m && m[1]) return m[1];
...
function detectAreaByDictionary(text) {
  if (!text) return null;
...
function extractCityLineArea(text) {
...
function genericAreaFromDescription(description) {
...
function determineLocation(plainText, description) {
  ...
  const dictArea = detectAreaByDictionary(plainText + " " + (description || ""));
